...
2. If uncommitted changes exist:
   - Analyze the changes and draft a commit message following **Korean Conventional Commits** format
     (see `/conventional-commit` skill or `.claude/docs/conventional-commit.md` for the full spec)
   - Commit to the **current branch** (not main) — never force-switch branches
   - **Ask the user for confirmation** before executing the commit
3. After commit, **ask the user for confirmation** before pushing to remote
...
**Application process:**
1. Collect all actionable findings
2. Present a summary of proposed changes to the user
3. **Wait for user approval** before applying any changes
4. After approval, apply the changes and commit using **Korean Conventional Commits** format
