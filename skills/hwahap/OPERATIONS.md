...
자유입력은 `Clarify`로 원문을 보존한다. 에이전트가 해석 선택지를 제안하고 사용자가 명시적으로
선택할 때까지 원래 질문을 확정하지 않는다. 영역 제외도 사용자가 선택하며, 적용 유지는 제외 제안을 철회한다.
빈 응답·UI 취소·timeout은 답변이나 승인으로 취급하지 않는다. `CONFIRM PLAN`·`SHIP`은 UI 질문으로 묻지 않는다.

새 대화형 run은 기획의 기준 source commit을 기록한다. 계획과 근거를 검토할 때 이 commit도 확인한다.
