...
재개 근거는 Hwahap 소유 스레드의 해제 확인, native 도구 복구처럼 새로 관찰한 호스트 상태여야 한다.
...
호스트의 에이전트 한도와 스레드 해제는 Hwahap이 보장할 수 없다. 생성 전 제공된 기능·알려진 capacity를
확인하고, 시험용 spawn으로 빈자리를 탐색하지 않는다. close/release 도구가 제공될 때만 완료 저장 확인 뒤
해당 Hwahap 소유 에이전트를 해제한다. interrupt만 있고 close/release가 없는 호스트에서는
완료·중단을 슬롯 반환으로 판단하지 않는다. 도구 제공 여부는 실행 환경마다 확인한다.
전역 한도를 변경하거나 무관한 작업을 닫아 우회하지 않는다. 범위 안에서 회복하지 못하면 외부 차단 요인을
알리고 같은 run을 보존한다. 상세 등록·실패·재개 호출은 실행 중인 MCP instructions를 따른다.
