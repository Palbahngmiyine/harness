...
등록한 뒤 반환된 JSON을 completion으로 전달하는 경로를 확인했다. 요약에는 요청 1개와 하위
에이전트 완료 1개, 사용량 보고 0개가 기록됐다. 실제 사용 토큰이나 청구금액은 확인하지 못했다.
이 관찰은 전체 계획·구현·최종 검토·PR 생성의 성공 증거가 아니다.

다음은 별도 실제 실행으로 검증해야 한다.

- Astra coordinator와 독립 Astra 최종 검토를 포함하는 전체 실행.
