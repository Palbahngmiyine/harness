...
class OrchestratorContractTests(unittest.TestCase):
    ...
    def test_direct_request_and_four_slot_lifecycle_are_separate_modes(self):
        ...
        self.assertIn("Only three input modes are authoritative", skill)
        self.assertNotIn("only to execute an implementation request against an explicitly provided PR/FAQ path", skill)
        self.assertNotIn("Require the user to provide the exact PR/FAQ path", skill)
        self.assertIn("status: request", skill)
        self.assertIn("init-request --request", skill)
        self.assertIn("init-goal --workspace <workspace> --goal-id <goal-id> --goal-spec", skill)
        self.assertIn("status: prfaq", skill)
        self.assertIn("init --workspace <workspace> --goal-id <goal-id> --spec <approved-prfaq-path>", skill)
        self.assertIn("Direct-request mode uses `init-request` and does not require PRFAQ", profile)
        self.assertIn("handoff-ready align-goal/v1 artifact", profile)
