...
class InstallerDirectoryBindingTests(InstallerFixture, unittest.TestCase):
    def test_directory_binding_replacement_uses_original_descriptors(self):
        ...
        for component in ("codex", "agents"):
            for fail_index in (1, 2):
                with self.subTest(component=component, fail_index=fail_index):
                    ...
                    def swap_before_create(name, flags, mode=0o777, *, dir_fd=None):
                        nonlocal calls
                        if flags & os.O_CREAT and flags & os.O_EXCL:
                            calls += 1
                            if calls == fail_index:
                                visible = workspace / ".codex" / (
                                    "agents" if component == "agents" else "")
                                moved = workspace / f"{component}-original"
                                visible.rename(moved)
                                visible.symlink_to(external, target_is_directory=True)
                        return original_open(name, flags, mode, dir_fd=dir_fd)

                    with patch.object(installer.os, "open", new=swap_before_create):
