...
def human_sections(view, summary, css, label, counts, metrics, metrics_html,
    ...
    return (
        f'<section id="summary" class="hero"><span class="eyebrow">Hwahap orchestration · '
        ...
        '</section></div><div class="supporting-pane"><section><span class="eyebrow">Remaining risk</span>'
        f'<h2>아직 남은 위험</h2><p class="section-intro">완료 판정과 별개로 아직 직접 검증하지 못했거나 새 승인이 '
        f'필요한 항목 {counts["risks"]}건입니다.</p><div class="cards">{deferred}</div></section></div></div>'
        ...
        f'{view.items(view.payload.get("next-actions"))}</section><section id="tests-metrics">'
        '<span class="eyebrow">Verification</span><h2>어떤 근거로 완료라고 판단했나</h2>'
        f'<div class="metrics-grid"><article class="card"><h3>검증 수치</h3><dl>{metrics_html}</dl>'
        f'<p>{token_html}</p></article><article class="card"><h3>최종 리뷰</h3>{final_html}</article></div>'
