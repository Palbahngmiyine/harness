...
def validate_approved_spec(workspace: Path, spec: dict, contract: dict, errors: list[str]) -> None:
    ...
    if lexical_path_has_symlink(source_path) or source_path.is_symlink() or not source_path.is_file():
        ...
        return
    if spec.get("status", "prfaq") not in {"prfaq", "request"}:
    ...
    try:
        ...
        if not stat.S_ISREG(source_info.st_mode) or source_info.st_nlink != 1:
            ...
            return
        if spec.get("status", "prfaq") == "request" and source_path.suffix.casefold() != ".md":
            errors.append("request source must be Markdown")
            return
    except OSError:
    ...
    try:
        ...
        metadata = frontmatter(source_path, expected_status=status,
                                error_code="HW_REQUEST_UNCONFIRMED" if status == "request" else "HW_SPEC_UNCONFIRMED")
    ...
    if (spec.get("status", "prfaq") not in {"prfaq", "request"}
            or metadata.get("status") != spec.get("status", "prfaq")
