...
class HwahapReportTests(unittest.TestCase):
    ...
    def test_decisions_and_completion_explain_evidence_with_5w3h(self) -> None:
        ...
        run["deviations"] = [{"summary": "보고서 누락", "impact": "값을 읽을 수 없음",
                              ...
                              "evidence_explanation": "ledger 변조 회귀가 누락된 값을 탐지하는지 확인함"}]
        context = {"scenario": "외부 감사자가 JSON actor를 실제 작성자 신원으로 믿는 상황",
                   "affected_scope": "외부로 공유된 완료 receipt와 승인 기록",
                   "impact": "서명이 없으면 위조된 actor와 실제 actor를 구분할 수 없음",
                   "decision_reason": "로컬 신뢰만 필요한지 서명된 외부 증명이 필요한지는 사용자가 정해야 함",
                   "evidence_relation": "trust boundary 문구는 서명 검증이 없다는 현재 경계만 뒷받침함",
                   "success_condition": "변조된 서명을 거부하고 모든 actor receipt의 서명을 검증함"}
        run["deferred_security"] = [{"summary": "외부 신원 증명", "reason": "actor 신원 서명 미검증",
                                      "next_action": "서명 receipt 계약 결정", "evidence": ["documented trust boundary"],
                                      "decision_context": context}]
        run["improvement_candidates"] = [{"status": "proposed", "summary": "후속 후보",
                                            "expected_effect": "오용 방지", "next_action": "사용자 승인",
                                            "evidence": ["final Ultra report"], "decision_context": context}]
        payload = report.build_payload("/tmp/work", contract, run, units, events, digests)
        digest = report.canonical_payload_digest(payload)
        text = report.render_report(payload, digest).decode()
        ...
        for label in ("누가 (Who)", "언제 (When)", "어디서 (Where)", "무엇을 (What)",
            ...
            self.assertIn(label, text)
        for evidence in ("payload-ledger binding tests", "documented trust boundary",
        ...
        self.assertIn("직접 연결된 구조화 receipt 필드는 없음", text)
        self.assertIn("외부 감사자가 JSON actor를 실제 작성자 신원으로 믿는 상황", text)
        self.assertIn("서명이 없으면 위조된 actor와 실제 actor를 구분할 수 없음", text)
        self.assertIn("왜 사용자가 결정해야 하는가", text)
        self.assertIn("로컬 신뢰만 필요한지 서명된 외부 증명이 필요한지는 사용자가 정해야 함", text)
        self.assertIn("변조된 서명을 거부하고 모든 actor receipt의 서명을 검증함", text)
        self.assertIn("ledger 변조 회귀가 누락된 값을 탐지하는지 확인함", text)
