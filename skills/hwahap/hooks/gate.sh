...
jq -n --arg id "$goal_id" --arg base "$base" --argjson revision "$revision" --argjson total "$units_total" --argjson passed "$passed" --argjson cached "$cached" --argjson first "$first_try" --argjson tokens "$tokens" --argjson orchestrator "$orchestrator" --argjson cache "$cache" --argjson cost "$cost" --argjson seconds "$seconds" --argjson parallel "$max_parallel" --argjson receipts "$receipts" --slurpfile g "$goal" '{goal_id:$id,revision:$revision,base_branch:$base,units_total:$total,units_passed:$passed,units_cached:$cached,first_try_pass:$first,tokens:{workers:$tokens,reviewers:0,facts:0,orchestrator:$orchestrator},units:[$g[0].units[]|select(.probe|not)|.id as $id|([$receipts[]|select(.unit==$id)]|{id:$id,cache_hit_ratio:(if (map(.input_tokens)|add//0)==0 then 0 else (map(.cached_input_tokens)|add)/(map(.input_tokens)|add) end),reasoning_ratio:(if (map(.output_tokens)|add//0)==0 then 0 else (map(.reasoning_output_tokens)|add)/(map(.output_tokens)|add) end)})],cache_hit_ratio:$cache,cost_usd:$cost,cost_per_passed_unit:(if $passed==0 then 0 else $cost/$passed end),seconds:$seconds,pr_url:null,deliver:"skipped:pending",improve:{signal:"none",started:false,reason:"not evaluated"},config:{worker_model:"gpt-5.6-luna",worker_effort:"high",reviewer_model:"gpt-5.6-terra",reviewer_effort:"high",max_parallel:$parallel}}' >.hwahap/summary.json
{
