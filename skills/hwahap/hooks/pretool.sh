...
template='codex exec -C <path> -s <mode> --ignore-user-config -m <model> -c model_reasoning_effort=<effort> --ephemeral; worker also requires --json -o <file> and the three fixed -c flags'
deny() {
  reason=$1
...
case "$command" in *' --ignore-user-config '*'-m '*' -c model_reasoning_effort='*' --ephemeral'*) ;; *) deny 'required codex exec flags are missing' ;; esac
if [ "$workdir" = . ]; then
...
if [ "$probe" = false ]; then
  ...
  repo_id=$(printf '%s' "$repo" | { command -v sha256sum >/dev/null 2>&1 && sha256sum || shasum -a 256; } | awk '{print $1}' | cut -c1-16)
  ...
  goal_hash=$(jq -S -c 'del(.review,.confirm)' .hwahap/goal.json | { command -v sha256sum >/dev/null 2>&1 && sha256sum || shasum -a 256; } | awk '{print "sha256:" $1}')
  ...
  render_hash=$(jq -r -f "$skill/jq/render.jq" .hwahap/goal.json | { command -v sha256sum >/dev/null 2>&1 && sha256sum || shasum -a 256; } | awk '{print "sha256:" $1}')
...
brief_hash=$({ command -v sha256sum >/dev/null 2>&1 && sha256sum "$brief" || shasum -a 256 "$brief"; } | awk '{print "sha256:" $1}')
if [ -f ".hwahap/out/$unit.patch" ]; then
  if [ "$(tail -n 1 ".hwahap/out/$unit.test.txt" 2>/dev/null)" = 'exit 0' ]; then
    if [ "$(head -n 1 ".hwahap/out/review/$unit.md" 2>/dev/null)" = 'verdict: pass' ]; then
      if [ "$(<".hwahap/out/$unit.brief.sha256")" = "$brief_hash" ]; then deny 'cached'; fi
    fi
  fi
fi
