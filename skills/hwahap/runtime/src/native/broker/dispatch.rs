...
use crate::session::{access_for, Access, NativeReceipt, SessionOutcome, SessionReceipt, SessionSpec};

impl NativeSessions {
    pub async fn execute(&self, spec: &SessionSpec) -> Result<SessionOutcome> {
        let started = Instant::now();
        let wanted = self.profiles.for_role(spec.role).clone();
        let run = self.store.read_run()?.ok_or_else(|| Error::Rejected("native dispatch has no active run".into()))?;
        let git = Git::open(&spec.cwd)?;
        let base_head = git.run_in(&spec.cwd, &["rev-parse", "HEAD"])?;
        let mut entropy = [0u8; 32];
        std::fs::File::open("/dev/urandom").and_then(|mut file| file.read_exact(&mut entropy))
        ...
        let brief = format!(
            "You are a Hwahap native worker for role {}. Work only in the absolute directory {}. \
             ...
             The host must start this task without inherited conversation history.\n\n{}",
            spec.role.as_str(), spec.cwd.display(), access, spec.prompt
        );
        let dispatch = NativeDispatch {
            dispatch_id: dispatch_id.clone(), run_id: run.run_id,
            role: spec.role.as_str().into(), profile: spec.role.profile().as_str().into(),
            unit: spec.unit.clone(), model: wanted.model.clone(), effort: wanted.effort.as_str().into(),
            cwd: spec.cwd.to_string_lossy().into_owned(), access: access.into(),
            prompt_digest: Digest::of_bytes(brief.as_bytes()).to_string(),
            plan_digest: run.plan_digest.map(|digest| digest.to_string()), base_head,
            brief, agent_id: None, stop_required: false,
        };
        let (sender, receiver) = oneshot::channel();
        {
            let mut guard = self.waiting.lock().map_err(super::poisoned)?;
            self.next_call()?;
            let pending = Pending { dispatch, completion: None };
            // Request is immutable evidence; pending also exists before anything is exposed.
            self.store.write_artifact(&format!("native-request-{dispatch_id}.json"), &json(&pending.dispatch)?)?;
            save(&self.store, &pending)?;
            *guard = Some(Waiting { pending, sender: Some(sender) });
        }
        let completion = match tokio::time::timeout(Duration::from_secs(self.timeout_secs), receiver).await {
            Ok(Ok(completion)) => completion,
            result => {
                self.expire()?;
                let reason = if result.is_err() { "deadline elapsed" } else { "continuation channel closed" };
        ...
        Ok(SessionOutcome {
            transcript: final_message.clone(), final_message,
            receipt: SessionReceipt::Native(NativeReceipt {
                dispatch_id, agent_id: completion.agent_id, profile: spec.role.profile(), role: spec.role,
                unit: spec.unit.clone(), model_requested: wanted.model, effort_requested: wanted.effort,
