...
Claude Code 2.1 계열과 Codex CLI 0.153 계열이 macOS 사용자 홈에 남기는 파일을 직접 읽어 정리했다. 두 도구의 공식 문서가 보장하는 형식이 아니므로 업데이트 뒤 파일명·테이블·필드가 달라질 수 있다.
...
- 하루 창: 사용자 시간대의 00:00부터 다음 날 00:00 전까지를 UTC로 바꿔 필터한다. KST(UTC+9)의 D일은 `[D-1일 15:00:00Z, D일 15:00:00Z)`다. epoch 밀리초는 `jq -n '"2026-01-31T15:00:00Z"|fromdateiso8601*1000'`처럼 구한다. 예시의 `+32400`과 `'+9 hours'`는 KST 오프셋이며 사용자 시간대에 맞춰 바꾼다.
- 시각 표기 세 가지: Claude Code 트랜스크립트는 UTC ISO 8601 문자열(밀리초 포함, 예 `2026-02-01T03:04:05.678Z`)이라 같은 형식의 문자열끼리 비교한다. `~/.claude/history.jsonl`과 Codex의 `*_ms` 컬럼은 epoch 밀리초다. Codex의 `created_at`과 `thread_turns.started_at`은 epoch 초다. 비교 전에 단위를 맞춘다.
...
| 경로 | 내용 | 비고 |
|---|---|---|
| `~/.codex/state_*.sqlite`의 `threads` | 스레드 목록. `id`(UUID), `thread_source`(`user`·`guardian_review`·`subagent`), `cwd`, `title`, `updated_at_ms`, `rollout_path`, `cli_version` | 오늘 필터는 `updated_at_ms`. 스레드는 며칠 전에 시작해 오늘까지 이어질 수 있다. 사건 후보는 `thread_source = 'user'`만이고 `guardian_review`·`subagent`는 제외한다. `-shm` 파일이 있으면 `mode=ro`로 열리고, 없으면 `&immutable=1`이 필요하다 |
| `~/.codex/thread_history_*.sqlite`의 `thread_items` | 항목. `thread_id`, `item_type`, `created_at_ms`, `item_json` | `-shm` 파일이 없으면 `mode=ro`만으로 열리지 않는다. 아래 예시는 `&immutable=1`을 바로 쓴다. `thread_history_*.sqlite-shm`이 있으면 `mode=ro`를 먼저 시도한다 |
...
전날 시작한 자율 실행이 오늘까지 이어지면 오늘 창에 `userMessage`는 0건이고 `agentMessage`·`commandExecution`·`fileChange`만 수천 건 남을 수 있다. `userMessage`만 세면 “오늘 Codex 사용 없음”이라는 오답이 나온다.
