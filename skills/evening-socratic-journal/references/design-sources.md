...
[지난 일기 조회와 월요일 검증](journal-review.md)의 직전 월~일 범위, 같은 기간 재검토 방지, 실행과 효과의 구분, 개선안 하나는 이 스킬의 운영 규칙이다. 특정 연구가 월요일이라는 요일이나 이 조합의 효과를 입증했다는 뜻이 아니다. 기록 부재를 행동 실패로 판정하지 않고, 사용자가 채택한 실천과 예측을 실제 후속 근거에 연결하는 데 목적이 있다. 스킬 호출 중의 조건부 동작이며 상주 수집기나 예약 자동화를 요구하지 않는다.
