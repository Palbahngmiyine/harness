...
- **S19 — Go testing and fuzzing.** [testing](https://pkg.go.dev/testing), especially T.Run, T.Cleanup, parallel tests, and fuzzing; [Go fuzzing](https://go.dev/doc/security/fuzz/); [coverage design](https://go.dev/blog/cover). Cleanup follows subtest completion; ordinary tests replay fuzz seeds, while active fuzzing is separate. Statement coverage is not branch coverage or MC/DC (GO-011/012/013). Failure-injection breadth and fixed budgets are retained package policy.
- **S20 — Go errors and foreign boundaries.** [errors](https://pkg.go.dev/errors), [cgo pointer passing](https://pkg.go.dev/cmd/cgo#hdr-Passing_pointers). Error inspection supports classified failure contracts; cgo imposes pointer retention/pinning obligations (GO-006/010). Neither mechanism establishes pure interfaces, safe arbitrary foreign pointers, or automatic resource cleanup.
