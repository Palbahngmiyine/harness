...
**C-002** `const T *` constrains access through that view; it does not prove deep immutability, exclusive ownership, or absence of changes through other aliases. Document pointer extent, initialization, alignment, lifetime, and who can mutate referenced storage. Copy or otherwise establish a stable snapshot at ingress when required; reject unstable borrowed state as a pure input.
