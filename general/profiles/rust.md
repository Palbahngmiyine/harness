...
**RS-003** Exclude shared `Cell`, `RefCell`, atomics, locks, lazy mutable state, and other `UnsafeCell`-based capabilities from domain inputs/results and captures. `Arc<T>` only provides shared ownership; inspect `T` and all reachable state. Immutable sharing is valid when the domain payload and operations respect the contract. Under an explicit FP-007 trusted-library contract, allocation and reference-count bookkeeping may be representation details, not mutable domain state. Never use pointer identity, reference counts, or `Weak` liveness to decide domain results; custom allocators and user destructors still need effect review.
